...
-- Applied after initial schema migration. Requires set_config('app.current_journal_id', ...)
-- and SET LOCAL row_security = on (see infrastructure/db/with-tenant.ts).

-- ── Helper: direct journalId column ──
-- Pattern: journalId = current_setting('app.current_journal_id', true)

-- ── Journal (tenant root: id matches session var) ──
...
-- ── Tables scoped via Submission.journalId ──
...
-- ── PaymentTransaction scoped via ApcInvoice.journalId ──
